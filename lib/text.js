...
const sentence = () => [...Array(Math.floor(Math.random()*(20-5)+5))].map(word).join(' ') + punctuation();
const paragraph = () => [...Array(Math.floor(Math.random()*(7-3)+3))].map(sentence).join(' ') + punctuation() + '\n';
